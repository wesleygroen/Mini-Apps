...
then
    while [ $i -lt ${cpn} ]; do
        ...
        prun -np 1 -$i -t 300 -script \$PRUN_ETC/prun-openmpi ${prog} >> ${out} &
    ...
    then
        while [ $i -lt ${nodes} ]; do
            let i+=2
            # echo prun -np $i -16 -script \$PRUN_ETC/prun-openmpi ${prog} \| tee -a ${out} &
            prun -np $i -${cpn} -t 300 -script \$PRUN_ETC/prun-openmpi ${prog} >> ${out} &
        ...
        while [ $i -lt ${cpn} ]; do
            let i+=2
            # echo prun -np $i -16 -script \$PRUN_ETC/prun-openmpi ${prog} \| tee -a ${out} &
            prun -np $nodes -$i -t 300 -script \$PRUN_ETC/prun-openmpi ${prog} >> ${out} &
